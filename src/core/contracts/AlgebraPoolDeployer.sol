...
contract AlgebraPoolDeployer is IAlgebraPoolDeployer {
  ...
  /// @inheritdoc IAlgebraPoolDeployer
  function deploy(address plugin, address token0, address token1, address customDeployer) external override returns (address pool) {
    require(msg.sender == factory);

    _writeToCache(plugin, token0, token1);
    bytes memory encodedParams;
    if (customDeployer == address(0)) {
      encodedParams = abi.encode(token0, token1);
    } else {
      encodedParams = abi.encode(customDeployer, token0, token1);
    }
    pool = address(new AlgebraPool{salt: keccak256(encodedParams)}());
  ...
  function _writeToCache(address plugin, address token0, address token1) private {
    assembly {
      ...
      token0 := and(token0, 0xFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF)
