...
import { decodePath, encodePath, encodeRoutePath } from './shared/path';
...
describe('Path', () => {
  ...
  let deployers = [ZeroAddress, tokenAddresses[2]];
  ...
  it('js encoding works as expected', async () => {
    let expectedPath =
      '0x' +
      [tokenAddresses[0], ZeroAddress, tokenAddresses[1]]
        .map((address) => address.slice(2).toLowerCase())
        .join('');
    expect(encodePath(tokenAddresses.slice(0, 2))).to.eq(expectedPath);

    expectedPath =
      '0x' +
      [tokenAddresses[0], ZeroAddress, tokenAddresses[1], ZeroAddress, tokenAddresses[2]]
        .map((address) => address.slice(2).toLowerCase())
        .join('');
    ...
    expectedPath =
      '0x' +
      [tokenAddresses[0], deployers[0], tokenAddresses[1], deployers[1], tokenAddresses[2]]
        .map((address) => address.slice(2).toLowerCase())
        .join('');
    expect(encodeRoutePath(tokenAddresses, deployers)).to.eq(expectedPath);
  });
  ...
  describe('#hasMultiplePools / #decodeFirstPool / #skipToken / #getFirstPool', () => {
    const encodedPath = encodeRoutePath(tokenAddresses, deployers);
    ...
    it('works on first pool', async () => {
      ...
      expect(firstPool.deployer).to.be.eq(deployers[0]);
    ...
    it('skips 1 item', async () => {
      ...
      expect(deployer).to.be.eq(deployers[1]);
  ...
  it('gas cost [ @skip-on-coverage ]', async () => {
    await snapshotGasCost(
      path.getGasCostOfDecodeFirstPool(encodeRoutePath([tokenAddresses[0], tokenAddresses[1]], [ZeroAddress]))
    );
  });
});
